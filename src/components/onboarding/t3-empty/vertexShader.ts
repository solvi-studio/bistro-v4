...
void main() {
  vec2 noiseCoord = uv * vec2(3.4, 3.1);
  ...
  float bumpThreshold = 0.2;
  float bumpHeight = smoothstep(bumpThreshold, bumpThreshold + 0.1, abs(combinedNoise));

  float displacement = combinedNoise * bumpHeight * 1.7;
  vec3 pos = vec3(position.x, position.y, position.z + displacement);

  float range = 1.8;
  float normalizedZ = displacement / range;
  float colorIntensity = abs(normalizedZ);
