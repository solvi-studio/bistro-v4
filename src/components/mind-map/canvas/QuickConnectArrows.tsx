...
function getDirOffset(
  dir: Direction,
  sw: number,
  sh: number, // source dimensions
  nw: number,
  nh: number, // new node dimensions
): { x: number; y: number } {
  switch (dir) {
    case "right":
      return { x: sw + GAP, y: (sh - nh) / 2 };
    case "left":
      return { x: -(nw + GAP), y: (sh - nh) / 2 };
    case "bottom":
      return { x: (sw - nw) / 2, y: sh + GAP };
    case "top":
      return { x: (sw - nw) / 2, y: -(nh + GAP) };
  }
}

const DIR_CONFIG: Record<
  Direction,
  { posClass: string; Icon: React.ComponentType<{ size?: number }> }
> = {
  top: {
    posClass:
      "top-0    left-1/2 -translate-x-1/2 -translate-y-[calc(100%+10px)]",
    Icon: ChevronUp,
  },
  right: {
    posClass:
      "right-0  top-1/2  -translate-y-1/2  translate-x-[calc(100%+10px)]",
    Icon: ChevronRight,
  },
  bottom: {
    posClass:
  ...
  left: {
    posClass:
      "left-0   top-1/2  -translate-y-1/2 -translate-x-[calc(100%+10px)]",
    Icon: ChevronLeft,
  },
};
